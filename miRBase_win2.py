...
class MiRBase():
    ...
    def load_data(self):
        ...
        print(f"load_organisms_txt execution time: {end - start}")

        name_miRNAdat = path + "/miRNA.dat.gz"

        start = timer()

        self.load_indexed_miRNAdat()
        #self.load_miRNAdat(name_miRNAdat)

        end = timer()
        print(f"Indexed load_miRNAdat execution time: {end - start}")
        ...
        print(f"Maps and zips exec time: {end - start}")
        ...
        print(f"get_hc execution time: {end - start}")
        ...
        #print(f"get_structures execution time: {end - start}")
        print(f"load_structures execution time: {end - start}")
        ...
        print(f"load_taxonomy exec time: {end - start}")

        with open('organisms_of_prec.mir', 'wb') as fh_orgprec_dump:
            pickle.dump(self.organisms_of_prec, fh_orgprec_dump)
        with open('taxonomy_of_prec.mir', 'wb') as fh_taxprec_dump:
            pickle.dump(self.taxonomy_of_prec, fh_taxprec_dump)

        #end = timer()
        #print(f"2 forloop exec time: {end - start}")

    def load_organisms_txt(self, file_path):
        spinner = LineSpinner("Fetching organisms")
        state = "WORKING"
    ...
    def load_taxonomy(self):
        with open('organisms_of_prec.mir', 'rb') as fh_orgprec_load:
            self.organisms_of_prec = pickle.load(fh_orgprec_load)
        fh_orgprec_load.close()
        print(self.organisms_of_prec["Caenorhabditis elegans"])
        #for elem in self.organisms_of_prec:
        #    print(elem)

        with open('taxonomy_of_prec.mir', 'rb') as fh_taxprec_load:
            self.taxonomy_of_prec = pickle.load(fh_taxprec_load)
        fh_taxprec_load.close()
        #print(self.taxonomy_of_prec["Metazoa"])

    def load_indexed_miRNAdat(self):
        with open('precursors_ID_pickle.mir', 'rb') as fh_precid_load:
            self.precursors_ID = pickle.load(fh_precid_load)
        fh_precid_load.close()

        with open('precursors_name.mir', 'rb') as fh_precname_load:
            self.precursors_name = pickle.load(fh_precname_load)
        fh_precname_load.close()

        with open('miRNAs_ID.mir', 'rb') as fh_mirnasid_load:
            self.miRNAs_ID = pickle.load(fh_mirnasid_load)
        fh_mirnasid_load.close()

    def load_structures(self):
        with open('structures.mir', 'rb') as fh_mirstruc_load:
            self.structures = pickle.load(fh_mirstruc_load)
    ...
    def load_miRNAdat(self, file_path):
                    ...
                    if line.startswith("ID"):  
                        ...
                        for miRNA_vals in products:
                            ...
                            if miRNA_vals[2] not in self.miRNAs_ID:
                                ...
                                #self.miRNAs_ID[miRNA_vals[2]] = mature.to_json()
                                #self.miRNAs_ID[miRNA_vals[2]] = mature.dumper()
                            else:
        ...
        # with open('precursors_ID.json', 'w') as j_precid:
        # #     json.dumps(self.precursors_ID, default=vars)
        #     precID = json.dumps(self.precursors_ID, default=vars, indent=4)
        #     j_precid.write(precID)

        #print(f"Normal: {self.precursors_ID}")

        with open('precursors_ID_pickle.mir', 'wb') as fh_precid_dump:
        #     json.dumps(self.precursors_ID, default=vars)
            #precID = pickle.dumps(self.precursors_ID)
            pickle.dump(self.precursors_ID, fh_precid_dump)
            #j_precid.write(precID)
        self.precursors_ID = {}
        with open('precursors_ID_pickle.mir', 'rb') as fh_precid_load:
            #print(f"Pickled: {pickle.load(p_precid)}")
            self.precursors_ID = pickle.load(fh_precid_load)


        with open('precursors_name.mir', 'wb') as fh_precname_dump:
        #     json.dumps(self.precursors_name, default=vars)
            pickle.dump(self.precursors_name, fh_precname_dump)
        self.precursors_name = {}
        with open('precursors_name.mir', 'rb') as fh_precname_load:
            #print(f"Pickled: {pickle.load(p_precid)}")
            self.precursors_name = pickle.load(fh_precname_load)
        ...
        #     json.dumps(self.miRNAs_ID, default=vars)
            pickle.dump(self.miRNAs_ID, fh_mirnasid_dump)
        self.miRNAs_ID = {}
        with open('miRNAs_ID.mir', 'rb') as fh_mirnasid_load:
            self.miRNAs_ID = pickle.load(fh_mirnasid_load)
    ...
    def get_structures(self):
        ...
        with open('structures.mir', 'wb') as fh_mirstruc_dump:
            pickle.dump(self.structures, fh_mirstruc_dump)
    ...
    def get_precursor(self, id=None, name="", organism_name="", organism_code="", tax_level="", chr="", start="",
                      end="", strand='', mirna_ID=""):
        ...
        if id is not None:
            ...
            try:
                ...
                for i in id:
                    for tax in self.taxonomy_of_prec:
                        if i in self.taxonomy_of_prec[tax]:
                            self.precursors_ID[i].taxonomy.append(tax)
                    print(self.precursors_ID[i].__dict__)
                    print(self.precursors_ID[i].info())
            except KeyError:
                print('ErRoR')
...
if __name__ == '__main__':
    ...
    parser.add_argument("-v", "--version", help = "database version", required = False, type = str, dest ="v", default="CURRENT")
    ...
    print(f"load_data execution time: {end - start}")
    #mirbase.get_coordinates(organism_code='aga')
 #ftp://mirbase.org/pub/mirbase/CURRENT/genomes/aga.gff3
    start = timer()

    mirbase.get_precursor(id=['MI0000001', 'MI0037052'])

    end = timer()
    print(f"get_precursor execution time: {end - start}")

    #for mi in mirbase.miRNAs_ID:
    #    print(mirbase.miRNAs_ID[mi].__dict__)
